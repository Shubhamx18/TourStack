...
// Function to get latest payment status for a booking
function getPaymentStatus($conn, $booking_id, $booking_type) {
    $stmt = $conn->prepare("SELECT status FROM payments WHERE booking_id = ? AND booking_type = ? ORDER BY created_at DESC LIMIT 1");
            ...
            <?php if (isset($_GET['debug']) && $_GET['debug'] == '1'): ?>
                <div class="admin-section" style="margin-bottom: 20px; background-color: #f8f9fa; border: 1px solid #dee2e6; padding: 15px; border-radius: 5px;">
                    ...
                    <p>
                        ...
                        <a href="admin/admin_bookings.php" class="btn btn-secondary" style="margin-left: 10px;">Hide Debug Info</a>
                    </p>
                </div>
            <?php else: ?>
                <?php if (isset($error_message)): ?>
                    ...
                    <tbody>
                                <?php while ($booking = $tour_bookings_result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <?php
                                        $payment_status = getPaymentStatus($conn, $booking['id'], 'tour');
                                        $payment_status_class = '';
                                        $payment_status_text = 'N/A';
                                        ...
                                        if ($payment_status) {
                                            switch ($payment_status) {
                                                case 'completed':
                                                    ...
                                                    break;
                                                case 'pending':
                                                    $payment_status_class = 'status-warning';
                                                    $payment_status_text = 'Pending';
                                                    break;
                                                case 'failed':
                                                    $payment_status_class = 'status-danger';
                                                    $payment_status_text = 'Failed';
                                                    break;
                                                case 'refunded':
                                                    $payment_status_class = 'status-info';
                                                    $payment_status_text = 'Refunded';
                                                    break;
                                            }
                                        }
                                        ?>
                                        <td><span class="booking-status <?php echo $payment_status_class; ?>"><?php echo $payment_status_text; ?></span></td>
                                        <td class="actions">
                        ...
                        <table class="admin-table">
                            ...
                            <tbody>
                                <?php while ($booking = $room_bookings_result->fetch_assoc()): ?>
                                    ...
                                    <?php
                                    $payment_status = getPaymentStatus($conn, $booking['id'], 'room');
                                    $payment_status_class = '';
                                    $payment_status_text = 'N/A';
                            ...
                            <tbody>
                                <?php while ($booking = $package_bookings_result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <?php
                                        $payment_status = getPaymentStatus($conn, $booking['id'], 'package');
                                        $payment_status_class = '';
                                        $payment_status_text = 'N/A';
